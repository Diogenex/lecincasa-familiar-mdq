...
const Availability = () => {
  ...
  const openWhatsApp = () => {
    ...
    if (dateRange?.from && dateRange?.to) {
      const fromDate = format(dateRange.from, "d 'de' MMMM", { locale: es });
      const toDate = format(dateRange.to, "d 'de' MMMM", { locale: es });
      message = `Hola, quisiera consultar disponibilidad para las fechas ${fromDate} a ${toDate}`;
    } else if (dateRange?.from) {
      const fromDate = format(dateRange.from, "d 'de' MMMM", { locale: es });
      message = `Hola, quisiera consultar disponibilidad desde el ${fromDate}`;
    } else {
      message = '¡Hola! Estoy consultando por Casa LeCin. ¿Podrían darme información sobre disponibilidad y precios?';
    }
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Seleccioná las fechas de tu interés y consultá disponibilidad por WhatsApp
          </p>
        </div>
        ...
        <div className="max-w-4xl mx-auto">
          <div className="bg-card rounded-2xl shadow-soft p-6 md:p-10">
            ...
            {dateRange?.from && (
              <div className="flex items-center justify-center gap-2 mb-6 p-4 bg-primary/10 rounded-lg">
                ...
                <span className="text-foreground font-medium">
                  {dateRange.to ? (
                    ...
                    <>Desde {format(dateRange.from, "d 'de' MMMM yyyy", { locale: es })}</>
                  )}
                </span>
              </div>
            )}
            ...
            <div className="flex justify-center" id="reservar">
              ...
              >
                ...
                {dateRange?.from ? 'Consultar estas fechas' : 'Consultar disponibilidad'}
              </Button>
            </div>
          </div>
        </div>
